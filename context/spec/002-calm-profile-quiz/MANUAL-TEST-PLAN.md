...
- [ ] **A1. Backend DB.** In `kumo_back-end` (branch `quiz-sequence`): `npm run docker:up`, then `npm run db:migrate`.
  **Expect:** Postgres on port 5433; migrations apply with no errors (including `add_lead_consent_scope_and_emails`).
- [ ] **A2. Backend env.** In `kumo_back-end/.env`: `NODE_ENV=development` (CORS allows any origin), `PORT=3001`, a real `RESEND_API_KEY`, `LANDING_URL=http://localhost:4321`.
  **Expect:** emails will actually be sent — only ever to your own addresses.
- [ ] **A3. Start backend.** `npm run dev`.
  **Expect:** `curl http://localhost:3001/health` → 200.
- [ ] **A4. Point the landing at local.** ⚠️ `calmisu_landing/.env.local` currently points `PUBLIC_API_BASE_URL` at the **production** Railway backend, which doesn't yet mint 15-day codes. Change it temporarily to `PUBLIC_API_BASE_URL=http://localhost:3001` (restore afterwards — see A-end).
- [ ] **A5. Start landing.** In `calmisu_landing` (branch `quiz-cta`): `npm run dev`.
  **Expect:** site opens at `http://localhost:4321`.
- [ ] **A6. Open Prisma Studio** in `kumo_back-end`: `npm run db:studio`. Keep the `leads`, `lead_emails` and `promo_codes` tables to hand.

---

## Phase B — Quiz page `/quiz/` (spec §2.1)

- [ ] **B1. No-JS render.** DevTools → Settings → *Disable JavaScript*, load `/quiz/`.
  **Expect:** H1, intro, trust line and "Start the quiz" button all visible. Re-enable JS.
- [ ] **B2. Title.** Check the tab title.
  **Expect:** `What's Your Anxiety Pattern? — 2-Minute Quiz | Calmisu`.
- [ ] **B3. Entry points.** From the homepage, find the quiz block (where reviews used to be) and the `Quiz` nav link.
  **Expect:** both lead to `/quiz/` (trailing slash); the nav link adds `?src=nav`. No reviews section.
- [ ] **B4. Progress + advance.** Start the quiz. Tap a single-select answer.
  **Expect:** advances immediately; progress reads `2 of 9` with no percentage.
- [ ] **B5. Multi-select.** Reach a multi-select question.
  **Expect:** tapping toggles options; a Continue button is required to advance.
- [ ] **B6. Back preserves answers.** Go Back two questions.
  **Expect:** your earlier choices are still selected.
- [ ] **B7. No storage.** DevTools → Application → Local Storage / Session Storage / Cookies for localhost.
  **Expect:** no quiz answers anywhere.
- [ ] **B8. Finish.** Complete all 9 questions.
  **Expect:** lands on `/quiz/result/?p=…&t=…&d=…` (plan encoded in the URL).
- [ ] **B9. "Not sure yet" tiebreak.** Retake; pick "Not sure yet" on Q1 and Q4/Q5 answers that point at one pattern (e.g. both sleep-flavoured).
...
- [ ] **C1. Minimal URL.** Open `/quiz/result/?p=sleep`.
  **Expect:** full profile — name, subtitle, three "why" paragraphs, two tools with one line each — before any email.
- [ ] **C2. Garbage URL.** Open `/quiz/result/?p=banana` and `/quiz/result/`.
  **Expect:** friendly fallback linking to `/quiz/`; no blank page, no console error.
- [ ] **C3. Noindex.** View source.
  **Expect:** `<meta name="robots" content="noindex, follow">`.
- [ ] **C4. Disclaimer.** Scroll the page.
  **Expect:** the "self-help tool, not a substitute for professional care" disclaimer is visible.
- [ ] **C5. Hydration warnings.** Console open, hard-reload `/quiz/result/?p=panic` on desktop and in device emulation (iPhone).
  **Expect:** no React hydration-mismatch warnings. *(Manual-only criterion.)*
- [ ] **C6. Copy tone.** Read every profile once (`p=panic|anxiety|racingThoughts|sleep|notSure`).
  **Expect:** no scores, no "you have", no "diagnosis / assessment / test / symptoms / condition / disorder".

---

## Phase D — Android gate, plan and code (spec §2.2)

- [ ] **D1. Phone preselect.** Open a result URL on desktop Chrome (Android UA emulation), then iPhone emulation.
  **Expect:** "Which phone do you use?" preselects Android / iPhone respectively; clicking the other option switches it.
- [ ] **D2. Consent blocks submit.** Enter `you+q1@gmail.com`, choose **Android**, leave consent unticked.
...
- [ ] **D3. Submit.** Tick consent, submit.
  **Expect:** plan appears in place, no reload. 7 rows matching your time of day and session length. A 10-digit code with a copy button.
- [ ] **D4. Code copy text.** Read the text around the code.
  **Expect:** it states both clocks separately — activate within **15 days**, **14 days** of PRO once redeemed — and says it's redeemed in the app after creating an account.
- [ ] **D5. Copy button.** Click copy, paste somewhere.
  **Expect:** exact code pasted.
- [ ] **D6. DB row.** In Prisma Studio → `leads`.
  **Expect:** one row with only `email, profile, source=quiz, platform=android, locale, consent=true, consentScope=quiz_plan_tips_v1, consentAt` (+ ids/tokens/timestamps). No answer columns. The linked `promo_codes` row has `expiresAt` ≈ now + 15 days, `durationDays` 14, `maxUses` 1.
...
- [ ] **F1. Same email twice.** Clear Local Storage; resubmit `you+q1@gmail.com` (Android).
  **Expect:** same code as D3; still one lead row.
- [ ] **F2. Expired code is replaced.** In Prisma Studio set that code's `expiresAt` to yesterday; clear storage; resubmit.
  **Expect:** a new code with a fresh 15-day expiry.
- [ ] **F3. Redeemed code → no new code.** Mark the current code as used (set its use count to its max, or redeem it in the app against the local backend); resubmit.
  **Expect:** no code shown.
- [ ] **F4. Rate limit.** Submit 6 times within an hour from the same browser.
...
- [ ] **G3. Sequence dry-run.** In Prisma Studio set `you+q1`'s `createdAt` to 2 days ago. From `kumo_back-end`, trigger the job once (ask the chat to write a one-off `tsx` script calling `runQuizSequence(prisma, logger)` from `src/jobs/quizSequence.ts` — don't commit it).
  **Expect:** Day 2 email arrives; a `lead_emails` row for day 2 exists. Run again → no second Day 2 email.
- [ ] **G4. Days 5 and 9.** Repeat G3 with `createdAt` at 5 and 9 days ago.
  **Expect:** each arrives once; every article link opens a real blog post and ends in `/`.
- [ ] **G5. Day 14.** Set `createdAt` 14 days ago **and** the code's `expiresAt` to ~24 h from now; run.
  **Expect:** "Your code expires tomorrow" arrives with the lead's own code.
- [ ] **G6. Day 14 suppression.** Using the iPhone lead (no code), or a redeemed code, or `expiresAt` 5 days out; run.
  **Expect:** no Day 14 email.
- [ ] **G7. Waitlist gets no sequence.** Use a lead from J1 with `createdAt` backdated 2 days; run.
  **Expect:** nothing sent.
- [ ] **G8. Unsubscribe.** Click unsubscribe in any sequence email.
  **Expect:** a confirmation page; `unsubscribedAt` set. Backdate for the next day and run → nothing sent.
- [ ] **G9. Copy status.** ⚠️ Day 2/5/9/14 bodies are an agent draft. Note anything to hand Julia; don't send to anyone else.

---

## Phase H — Blog CTAs (spec §2.4)

- [ ] **H1.** Open each of the 7 posts under `/blog/`.
  **Expect:** a quiz CTA between the article body and the download block; at least 4 different CTA texts across the 7.
- [ ] **H2.** Hover/click a CTA.
...
- [ ] **K2. Real CORS.** On `https://calmisu.com/quiz/`, take the quiz, submit a fresh address (Android).
  **Expect:** no CORS error in the console; code shown; its expiry ~15 days out.
