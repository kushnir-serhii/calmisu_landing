...
const ReviewCard = ({ quote, author }: ReviewCardProps) => (
  <div className="flex p-6 flex-col items-center gap-6 rounded-3xl bg-brand-light">
    <Stars />
    <p className="text-foreground text-center font-body text-xl md:text-2xl font-normal leading-[140%]">{quote}</p>
    <p className="text-muted-foreground text-center font-body text-lg font-light leading-[140%]">{author}</p>
  </div>
);
...
const ReviewsSection = () => {
  return (
    <section className="relative w-full px-6 md:px-[120px] py-20 rounded-3xl overflow-hidden">
      {/* Background */}
      <img src="/images/reviews-water-bg.webp" alt="" className="absolute inset-0 w-full h-full object-cover blur-[7.5px] scale-110" loading="lazy" />
      <div className="absolute inset-0 bg-foreground/30" />

      <div className="relative z-10 flex flex-col lg:flex-row items-end gap-5">
        {/* Title */}
        <h2 className="flex-1 text-background font-display text-4xl md:text-5xl lg:text-[60px] font-normal leading-[100%]">
          Words from people who found calm with Calmisu
        </h2>

        {/* Review columns */}
        <div className="flex flex-col sm:flex-row items-end gap-5">
          <div className="flex flex-col gap-5 w-full sm:w-[387px]">
